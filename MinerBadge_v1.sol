 ...
 *   GOD       — Legendary badge already claimed AND top-25 leaderboard position.
 *               Capped at MAX_GOD (1) mint ever — only one player can hold this.
 *   LEGENDARY — totalMined >= 95,000. No leaderboard requirement.
 *               Capped at MAX_LEGENDARY (10) mints ever.
 *   RARE      — totalMined >= 9,800.
 *               Available to ALL qualifying players.
 *   COMMON    — totalMined >= 400.
 *
 * Rules:
 *   - Players can claim each tier badge independently as they reach it.
 *   - One badge NFT per tier per wallet (max 4 badges total).
 ...
 *   H-02  GOD snapshot race documented: claimGod() checks leaderboard
 *         membership at call time. Players must be in the top-25 at the
 ...
 *         behaviour for a casual game. The MAX_GOD cap (1) limits total
 *         exposure to 1 badge.
...
contract MinerBadge {
    ...
    enum Tier { Common, Rare, Legendary, God }

    uint256 public constant COMMON_THRESHOLD    =     400;
    uint256 public constant RARE_THRESHOLD      =   9_800;
    uint256 public constant LEGENDARY_THRESHOLD =  95_000;
    // H-01: hard cap — only 10 Legendary badges can ever be minted
    uint256 public constant MAX_LEGENDARY       =      10;
    // GOD tier: requires Legendary already claimed + top-25 leaderboard.
    // Only 1 GOD badge can ever exist.
    uint256 public constant MAX_GOD             =       1;
    ...
    /// @notice Claim your Legendary badge. Requires 95,000 cBALL mined. No leaderboard requirement.
    ...
    /// @notice Claim the GOD badge. Requires Legendary already claimed + top-25 leaderboard.
    ///         Only ONE GOD badge can ever exist (MAX_GOD = 1). First qualifying
    ///         top-25 Legendary holder to call this wins it permanently.
    ...
    function _claimTier(Tier tier) internal {
        ...
        if (tier == Tier.Common) {
            ...
            // H-02: open to all who hit the threshold — no leaderboard requirement
            require(totalMined >= RARE_THRESHOLD, "Mine at least 9,800 cBALL to claim Rare");
        } else if (tier == Tier.Legendary) {
            // Legendary: threshold only, no leaderboard requirement
            require(totalMined >= LEGENDARY_THRESHOLD,  "Mine at least 95,000 cBALL to claim Legendary");
            require(legendaryCount < MAX_LEGENDARY,     "Legendary badge cap reached (10)"); // H-01
        } else {
            // God: must hold Legendary badge + be in top-25 leaderboard
            require(hasClaimedTier[msg.sender][Tier.Legendary], "Must hold Legendary badge to claim God");
            require(game.isOnLeaderboard(msg.sender),           "Must be in top-25 leaderboard for God");
            require(godCount < MAX_GOD,                         "GOD badge already claimed - only 1 can ever exist");
        }
        ...
        if (tier == Tier.God)       godCount++;
        else if (tier == Tier.Legendary) legendaryCount++;
        ...
        )
    {
        ...
        legendaryEligible = totalMined >= LEGENDARY_THRESHOLD
                            && legendaryCount < MAX_LEGENDARY;                                     // H-01, no leaderboard req
        godEligible       = hasClaimedTier[player][Tier.Legendary]
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory tierName  = t == Tier.God ? "God" : t == Tier.Legendary ? "Legendary" : t == Tier.Rare ? "Rare" : "Common";
        string memory tierEmoji = t == Tier.God ? unicode"\u2604" : t == Tier.Legendary ? unicode"\u2B50" : t == Tier.Rare ? unicode"💎" : unicode"\u26CF";
